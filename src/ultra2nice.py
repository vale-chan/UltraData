...
def main():
    ...
    cli_args.add_argument('--timelabels', type=str, action='store', required=False, default=[], nargs='+',
                          help="Enter the timelabels, e.g.: 2018 2019 2020")
    cli_args.add_argument('--verbosity', type=int, action='store', required=False, default=3,
                          help='Choose the verbosity level (1 = low, 4 = high') 
    cli_args.add_argument('--clearchache', type=bool, action='store_true', required=False, default=False,
                          help='Choose "True" or "False" to clear the cache. Necessary when plot sizing is changend.')               
    ...
    main(ARGS.plottype, [ARGS.pathtodata], "niceconfig.yml", codebook, ARGS.outputname, ARGS.clearchache, ARGS.saveas, ARGS.timelabels, ARGS.verbosity)
